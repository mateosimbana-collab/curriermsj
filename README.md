...
Sistema de atencion y registro de envios para paquetes que viajan desde Estados Unidos hacia Ecuador. El canal principal es WhatsApp Business Cloud API, el backend es Flask y los datos se guardan en Supabase. Incluye un panel administrativo en React + Chakra UI. Telegram queda como soporte legacy temporal documentado.
...
- Panel administrativo con 5 vistas (dashboard, clientes, envios, reportes, observabilidad).
...
- `clientes`: clientes registrados via WhatsApp.
...
erDiagram
    ...
    CLIENTE ||--o{ ENVIO : telefono_remitente
    CLIENTE ||--o{ REPORTE : phone_number
```
...
Telegram queda documentado solo como soporte temporal. El codigo actual del repositorio ya trabaja con WhatsApp (`app.py`) y los archivos Telegram legacy (`bot.py`, `handlers/`, `db.py`) fueron eliminados por estar completamente reemplazados.

Si necesitas reactivar Telegram:

1. Crea el bot con BotFather.
2. Copia el token en `TELEGRAM_BOT_TOKEN`.
3. Restaura el codigo Telegram antiguo desde historial Git si existe.
4. Mantenlo separado del flujo WhatsApp.

## Pruebas

El backend incluye 56 tests unitarios con pytest:
